...
- ALL combinators — sync and async — are EXTENSION METHODS in per-family classes; the types keep only state, properties, factories, `Combine`, conversions and `ToString`. NEVER add a same-name instance combinator: an applicable instance member blocks the ENTIRE extension family for that name (`Tap(Action<…>)` and `Map`'s open `TResult` would silently swallow async lambdas). Creed exceptions on the types: the properties, and the sync `AsFailure` (`Suspicious<TValue>.AsFailure<TResult>()` as an extension would need TWO explicit type args — C# has no partial type-argument inference; for the same reason `Match<object>(…)`-style explicit calls don't exist on two-type-param extensions — use explicit lambda return types like `static object (v) => v`).
