...
5. **Pack.** Run `zsh Nuget/pack.zsh` (the scripts intentionally lack the executable bit — always invoke via `zsh`, never `chmod +x`) and verify `Rumrunner0.BackToReality.Suspicious/bin/Release/Rumrunner0.BackToReality.Suspicious.<full version>.nupkg` now exists.
